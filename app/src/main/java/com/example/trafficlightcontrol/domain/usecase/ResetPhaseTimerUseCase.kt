...
class ResetPhaseTimerUseCase @Inject constructor(
    private val trafficLightRepository: TrafficLightRepository,
    private val userRepository: UserRepository,
    private val logRepository: LogRepository
) {
    suspend operator fun invoke(direction: Direction): Result<Unit> {
        ...
        val result = trafficLightRepository.resetPhaseTimer(direction)
        ...
        result.onSuccess {
            ...
            logRepository.addLog(
                ActionLog(
                    ...
                    description = "Reset timer for ${direction.name} light",
                    previousState = "Phase: ${currentState.currentPhase.name}, Time: ${currentState.remainingTime}s",
                    newState = "Phase: ${currentState.currentPhase.name}, Time reset"
                )
            )
        }
